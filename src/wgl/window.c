...
int glwtWindowCreateWGL(GLWTWindow *win, GLWTWindow *share)
{
    int context_attribs[] = {
        ...
        ((glwt.api & GLWT_PROFILE_DEBUG) ? WGL_CONTEXT_DEBUG_BIT_ARB : 0) |
            ((glwt.api & GLWT_PROFILE_ROBUSTNESS)
    ...
    win->wgl.context = wglCreateContextAttribsARB(
        win->win32.hdc, share ? share->wgl.context : 0, context_attribs);
    if(!win->wgl.context)
    {
...
{
    if(!wglMakeContextCurrentARB(win ? win->win32.hdc : 0,
                                 win ? win->win32.hdc : 0,

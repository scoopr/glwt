...
static int mapKeyMod(unsigned int state)
{
    ...
    return 0 | ((state & ShiftMask) ? GLWT_MOD_SHIFT : 0) |
...
{
    return 0 | ((state & Button1Mask) ? (1 << 0) : 0) |
...
static int translate_key(int xkeysym)
{
    ...
    {
        case XK_BackSpace:
            return GLWT_KEY_BACKSPACE;
        case XK_Tab:
            return GLWT_KEY_TAB;
        case XK_Return:
            return GLWT_KEY_RETURN;
        case XK_Escape:
            return GLWT_KEY_ESCAPE;
        case XK_space:
            return GLWT_KEY_SPACE;

        case XK_plus:
            return GLWT_KEY_PLUS;
        case XK_comma:
            return GLWT_KEY_COMMA;
        case XK_minus:
            return GLWT_KEY_MINUS;
        case XK_period:
            return GLWT_KEY_PERIOD;
        case XK_slash:
            return GLWT_KEY_SLASH;
        ...
        case XK_KP_Enter:
            return GLWT_KEY_KEYPAD_ENTER;

        case XK_Up:
            return GLWT_KEY_UP;
        case XK_Down:
            return GLWT_KEY_DOWN;
        case XK_Left:
            return GLWT_KEY_LEFT;
        case XK_Right:
            return GLWT_KEY_RIGHT;
        case XK_Page_Up:
            return GLWT_KEY_PAGE_UP;
        case XK_Page_Down:
            return GLWT_KEY_PAGE_DOWN;
        case XK_Home:
            return GLWT_KEY_HOME;
        case XK_End:
            return GLWT_KEY_END;
        case XK_Insert:
            return GLWT_KEY_INSERT;
        ...
        case XK_Shift_L:
            return GLWT_KEY_LSHIFT;
        case XK_Shift_R:
            return GLWT_KEY_RSHIFT;
        case XK_Control_L:
            return GLWT_KEY_LCTRL;
        case XK_Control_R:
            return GLWT_KEY_RCTRL;
        case XK_Alt_L:
            return GLWT_KEY_LALT;
        case XK_Alt_R:
            return GLWT_KEY_RALT;
        case XK_Super_L:
            return GLWT_KEY_LSUPER;
        case XK_Super_R:
            return GLWT_KEY_RSUPER;
        case XK_Menu:
            return GLWT_KEY_MENU;
        case XK_ISO_Level3_Shift:
            return GLWT_KEY_ALTGR;
        case XK_Num_Lock:
            return GLWT_KEY_NUM_LOCK;
        case XK_Caps_Lock:
            return GLWT_KEY_CAPS_LOCK;
        case XK_Scroll_Lock:
            return GLWT_KEY_SCROLL_LOCK;
...
static unsigned int decode_utf8(const unsigned char *data, int len)
{
    if(len == 0)
        return 0;
    else if(len == 1 && (data[0] & 0x80) == 0)
        return data[0];
    else if(len == 2 && (data[0] & 0xe0) == 0xc0 && (data[1] & 0xc0) == 0x80)
        return ((data[0] & 0x1f) << 6) | (data[1] & 0x3f);
    else if(len == 3 && (data[0] & 0xf0) == 0xe0 && (data[1] & 0xc0) == 0x80 &&
            (data[2] & 0xc0) == 0x80)
        return ((data[0] & 0x0f) << 12) | ((data[1] & 0x3f) << 6) |
               (data[2] & 0x3f);
    else if(len == 4 && (data[0] & 0xf7) == 0xf0 && (data[1] & 0x3f) == 0x80 &&
            (data[2] & 0x3f) == 0x80 && (data[3] & 0x3f) == 0x80)
        return ((data[0] & 0x7) << 18) | ((data[1] & 0x3f) << 12) |
...
static int xlib_handle_event()
{
    ...
    while(XCheckIfEvent(glwt.x11.display, &event, xlib_event_predicate, NULL))
    {
        ...
        GLWTWindow *win = 0;
        if(XFindContext(glwt.x11.display,
                        event.xany.window,
                        glwt.x11.xcontext,
                        (XPointer *)&win) != 0)
            continue;
        ...
        switch(event.type)
        {
            ...
            case UnmapNotify:
                if(win->win_callback)
                {
                    GLWTWindowEvent e;
                    e.window = win;
                    e.type = event.type == MapNotify ? GLWT_WINDOW_SHOW
                                                     : GLWT_WINDOW_HIDE;
                    e.dummy.dummy = 0;
                    win->win_callback(win, &e, win->userdata);
                }
            ...
            case KeyRelease:
                if(win->win_callback)
                {
                    GLWTWindowEvent e;
                    e.window = win;
                    e.type = event.type == KeyPress ? GLWT_WINDOW_KEY_DOWN
                                                    : GLWT_WINDOW_KEY_UP;
                    e.key.keysym = translate_key(XkbKeycodeToKeysym(
                        glwt.x11.display, event.xkey.keycode, 0, 0));
                ...
                if(event.type == KeyPress && !XFilterEvent(&event, None))
                {
                    ...
                    len = Xutf8LookupString(win->x11.xic,
                       ...
                       (status == XLookupBoth || status == XLookupChars))
                    {
                        ...
                        e.character.unicode =
                            decode_utf8((unsigned char *)buffer, len);
            ...
            case FocusOut:
                if(win->win_callback)
                {
                    GLWTWindowEvent e;
                    e.window = win;
                    e.type = event.type == FocusIn ? GLWT_WINDOW_FOCUS_IN
            ...
            case ButtonRelease:
                if(win->win_callback)
                {
                    GLWTWindowEvent e;
                    e.window = win;
                    e.type = event.type == ButtonPress ? GLWT_WINDOW_BUTTON_DOWN
                                                       : GLWT_WINDOW_BUTTON_UP;
                    e.button.x = event.xbutton.x;
                    e.button.y = event.xbutton.y;
                    e.button.button =
                        event.xbutton.button; // todo: make these consistent on
                                              // different platforms

...
{
    return (GetKeyState(VK_SHIFT) & 0x8000 ? GLWT_MOD_SHIFT : 0) |
...
static int translate_key(WPARAM wParam, LPARAM lParam)
{
    ...
    {
        case VK_SHIFT:
            return ((unsigned)((lParam & 0x01ff0000) >> 16) ==
                    MapVirtualKeyW(VK_RSHIFT, MAPVK_VK_TO_VSC))
                       ? GLWT_KEY_RSHIFT
                       : GLWT_KEY_LSHIFT;
        case VK_CONTROL:
        ...
        case VK_RETURN:
            return (lParam & 0x01000000) ? GLWT_KEY_KEYPAD_ENTER
                                         : GLWT_KEY_RETURN;
...
static unsigned int utf16_decode(unsigned int utf16)
{
    ...
    if(lo < 0xD8000 || lo > 0xDFFF)
        ...
        return ((lo & 0x3FF) << 10) | (hi & 0x3FF) + 0x10000;
}

LRESULT CALLBACK
glwtWin32WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
        ...
        switch(uMsg)
        {
            ...
            case WM_MOUSEHWHEEL:
                if(win->win_callback)
                {
                    ...
                    event.window = win;
                    event.type =
                        (uMsg == WM_LBUTTONUP || uMsg == WM_MBUTTONUP ||
                         uMsg == WM_RBUTTONUP || uMsg == WM_XBUTTONUP ||
                    ...
                    event.button.button =
                        (uMsg == WM_LBUTTONUP || uMsg == WM_LBUTTONDOWN)
                            ? 0
                            : (uMsg == WM_MBUTTONUP || uMsg == WM_MBUTTONDOWN)
                                  ? 1
                                  : (uMsg == WM_RBUTTONUP ||
                                     uMsg == WM_RBUTTONDOWN)
                                        ? 2
            ...
            case WM_MOUSEHOVER:
                if(win->win_callback)
                {
                    ...
                    event.window = win;
                    event.type =
                        (win->win32.hover == 1 ? GLWT_WINDOW_NO_EVENT
            ...
            case WM_SHOWWINDOW:
                if(win->win_callback)
                {
                    ...
                    event.window = win;
                    event.type =
                        (wParam == TRUE ? GLWT_WINDOW_SHOW : GLWT_WINDOW_HIDE);
                    event.dummy.dummy = 0;
                    win->win_callback(win, &event, win->userdata);
                }
